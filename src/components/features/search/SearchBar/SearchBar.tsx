...
export default function SearchBar({ brands }: SearchBarProps) {
  const router = useRouter()
  const searchBarRef = useRef<HTMLDivElement>(null)
  const [activeAudiences, setActiveAudiences] = useState<Audience[]>([Audience.MEN])
  ...
  useEffect(() => {
    ;(async () => {
      const { data: searchProducts, error } = await action.search.getSearchProductsAction(activeAudiences, searchQuery)
  ...
  const handleClickBrand = (brand: string) => {
    router.push(`/collections?${util.audience.audiencesToUrlParams(activeAudiences)}&brands=${brand}`)
    setOpen(false)
  }

  const handleClickSeeMoreResults = () => {
    router.push(`/search/results?q=${searchQuery.replaceAll(" | ", "+")}&${util.audience.audiencesToUrlParams(activeAudiences)}`)
    setOpen(false)
  }
  ...
  return (
    <>
      ...
      {/* //---------- SEARCH MENU ----------// */}
      <div ref={searchBarRef} className={`${styles["search-bar"]} ${open ? styles.open : ""}`}>
        <div className={styles["container"]}>
          ...
          <div className={styles["search-container"]}>
            ...
            <input
              ...
              type="text"
              placeholder={"Recherche pour " + util.audience.audiencesToLabel(activeAudiences)}
            />
          ...
          <div className={styles["menu"]}>
            ...
            <div className={styles["right-section"]}>
              <SearchBarFilter activeAudiences={activeAudiences} setActiveAudiences={setActiveAudiences} />
              ...
              {searchProducts ? (
                <div className={styles["sneakers-grid"]}>
                  {searchProducts.products.map((sneaker, index) => (
                    <div key={index} onClick={() => setOpen(false)} className={styles["sneaker-container"]}>
