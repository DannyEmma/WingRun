...
export default async function SearchResultsPage({ searchParams }: { searchParams: Promise<Record<string, string>> }) {
  const {
    q: query,
    adults: adutlsParam,
    kids: kidsParam,
  ...
  const audiences: Audience[] =
    (adutlsParam?.split(",").filter((v) => v !== ",") as Audience[]) || (kidsParam?.split(",").filter((v) => v !== ",") as Audience[]) || []
